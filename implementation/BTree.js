...
BTreeNode.prototype.insertKey = function (key) {
  ...
  while (pos > 0 && keys[pos - 1][0] > key[0]) {
    keys[pos] = keys[pos - 1];
    pos--;
  }
...
BTreeNode.prototype.preorderPrint = function (level, child) {
  if (!this) {
    return '\n';
  }
  ...
  for (var i = 0; i <= this.keyCount(); i++) {
    ...
    if (this._childs[i]) {
      result += '\n' + this._childs[i].preorderPrint(level + 1, i);
    }
  }
...
BTree.prototype.preorderPrint = function () {
  return this._root.preorderPrint(0, 0);
};

BTreeNode.prototype.inorderPrint = function () {
  if (!this) {
    return [];
  }
...
BTreeNode.prototype.postorderPrint = function () {
  ...
  if (this._childs[0]) {
    result = result.concat(this._childs[0].postorderPrint());
  }

  for (var i = 1; i <= this.keyCount(); i++) {
    if (this._childs[i]) {
      result = result.concat(this._childs[i].postorderPrint());
    }
    result.push(this._keys[i - 1]);
  }

  return result;
};

BTree.prototype.postorderPrint = function () {
  return this._root.postorderPrint();
};

// Add the following code to the BTreeNode class
BTreeNode.prototype.search = function (key) {
  for (var i = 0; i < this.keyCount(); i++) {
    if (key === this._keys[i][0]) {
      // Key found, return the associated value (meaning)
      return this._keys[i];
    } else if (key < this._keys[i][0]) {
      // The key may be in the child node
      if (this._childs[i]) {
        return this._childs[i].search(key);
      } else {
  ...
  // Check the rightmost child if key is greater than all keys in the node
  if (this._childs[this.keyCount()]) {
    return this._childs[this.keyCount()].search(key);
  }
...
// Add the following code to the BTree class
...
// console.log(btree.preorderPrint());
// console.log('inorder print')
// console.log(btree.inorderPrint());
// console.log('postorder print')
// console.log(btree.postorderPrint());
